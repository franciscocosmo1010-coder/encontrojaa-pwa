// EncontroJá - Perfis fake + Auto-like + Monetização (3 planos ≤ R$20) - v8
...
function generateBots(n=100){
  ...
  for(let i=0;i<n;i++){
    ...
    arr.push({ id, gender, name, age, km, online, bio:sample(bios), sign:sample(signs), body:sample(bodies), photos:[img] });
  }
  return arr;
}
...
function refillIfNeeded(){
  ...
  if(!m.nextRefillAt || m.nextRefillAt < now()){
    m.dayLikesUsed = 0; m.dayMsgsUsed = 0;
    m.lastRefillAt = now();
...
function timeLeftToRefill(){ const ms=Math.max(0,state.monet.nextRefillAt-now()); const h=Math.floor(ms/3600000); const m=Math.floor((ms%3600000)/60000); return `${h}h ${m}m`; }
...
function updateRangeBadges(){
  ...
  const badge = $('#planBadge'); if(badge){ (state.subscription==='premium'?badge.classList.remove('hidden'):badge.classList.add('hidden')); }
  const likesBtn = $('#btnLikesYou'); if(likesBtn){ likesBtn.textContent = state.subscription==='premium' ? 'Quem curtiu você' : 'Quem curtiu (Premium)'; }
}
function buildQueue(){
...
function renderDeck(){
  const deck = $('#profiles'); if(!deck) return;
  deck.innerHTML = ''; updateRangeBadges(); buildQueue();
  if(state.queue.length===0){ deck.innerHTML = `<div class="card">Sem mais perfis nestes filtros. Tente ampliar a idade/raio.</div>`; return; }
  const p = state.queue[0];
  const el = document.createElement('div'); el.className='card-profile';
  el.innerHTML = `
...
function renderMatches(){
  ...
  box.innerHTML='';
  if(state.matches.length===0){ box.innerHTML='<div class="card">Você ainda não tem matches.</div>'; return; }
  state.matches.forEach(m=>{
    const p = state.bots.find(b=>b.id===m.id); if(!p) return;
    const chat = state.chats[m.id] || []; const last = chat[chat.length-1];
  ...
  $$('#matchesList .btn').forEach(btn=>{
    btn.onclick = ()=>{
      const id = btn.dataset.id; const input = document.getElementById('msg_'+id);
      const txt = (input?.value||'').trim(); if(!txt) return;
      if(!canSendMessage()){ const left = timeLeftToRefill(); return requirePremium(`Limite de mensagens atingido (${state.monet.freeMaxMsgsPerDay}/dia). Próximo recarregamento em ${left}.`); }
      sendMessageTo(id, txt); input.value=''; renderMatches();
    };
  });
}
function renderLikes(){
  const wrap = $('#likesList'); if(!wrap) return; wrap.innerHTML = '';
  if(state.subscription!=='premium'){
    wrap.innerHTML = `<div class="card"><b>Recurso Premium</b><br/>Veja quem curtiu você e dê like de volta instantaneamente.<div class="row" style="margin-top:8px"><button class="btn" id="upsellLikes">Assinar Premium</button></div></div>`;
    $('#upsellLikes').onclick=()=>show('#viewPremium'); return;
  }
  if(state.likesIncoming.length===0){ wrap.innerHTML = '<div class="card">Ninguém curtiu você ainda. Faça um Boost! 🚀</div>'; return; }
  state.likesIncoming.forEach(id=>{
    const p = state.bots.find(b=>b.id===id); if(!p) return;
    const div = document.createElement('div'); div.className='tile';
    div.innerHTML = `<img src="${p.photos[0]}"><div class="info"><div class="name">${p.name}, ${p.age}</div><div class="row"><button class="btn btn-like" data-id="${p.id}">Curtir de volta</button></div></div>`;
...
function like(){
  ...
  if(!canLike()){ const left=timeLeftToRefill(); return requirePremium(`Limite de likes atingido (${state.monet.freeMaxLikesPerDay}/dia). Próximo recarregamento em ${left}.`); }
  consumeLike(); const p = state.queue.shift(); likeById(p.id, false);
}
function likeById(id, fromLikesList){
  ...
  if(likedMe){ state.matches.push({id, ts: now()}); state.likesIncoming = state.likesIncoming.filter(x=>x!==id); state.chats[id]=(state.chats[id]||[]); state.chats[id].push({from:'them', text:'Oi! 😄', ts:now()}); }
  save(); if(fromLikesList){ renderLikes(); renderMatches(); } else { renderDeck(); renderMatches(); }
}
...
$('#btnStart').onclick = ()=>{
  ...
  state.me = { name: $('#name').value || 'Eu', age: parseInt($('#age').value)||25, bio: $('#bio').value||'', gender: $('#gender').value, pref: $('#pref').value, photos: [] };
  if(!state.bots || state.bots.length===0){ state.bots = generateBots(100); }
  botsAutoLikeMe(); refillIfNeeded(); save();
...
// Planos (≤ R$20) - troque pelos links reais
const planLinks = {
...
function init(){
  ...
  ['filterMinAge','filterMaxAge','filterKm'].forEach(id=>{
    ...
    el.addEventListener('input', ()=>{
      if(id!=='filterKm'){ $('#ageRange').textContent = `${$('#filterMinAge').value}-${$('#filterMaxAge').value}`; }
      else{ $('#kmRange').textContent = `${$('#filterKm').value}km`; }
    });
  });
}
init();
